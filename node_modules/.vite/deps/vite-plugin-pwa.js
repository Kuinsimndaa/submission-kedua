...
async function loadWorkboxBuild() {
  try {
    const workbox = await import("./build-WTWOPDJO.js");
...
async function generateInjectManifest(version, options, viteOptions) {
  ...
  await import("./vite-build-BGK4YAIU-WMWWWJZT.js").then(({ buildSW }) => buildSW(version, options, viteOptions, loadWorkboxBuild()));
}
  ...
  return {
    ...
    async configResolved(config) {
      ...
      if (ctx.options.pwaAssets && !ctx.options.pwaAssets.disabled) {
        ctx.pwaAssetsGenerator = import("./generator-QYK42GLH-QUIASKTQ.js").then(({ loadInstructions }) => loadInstructions(ctx)).catch((e) => {
          console.error([
            "",
            cyan(`PWA v${ctx.version}`),
